...
class Messenger():
    ...
    def __getMessageDeliveryState(self, result):
        ...
        """
        state = 0
        if   (self.__wasDeliverySuccessful(result) == True):          state |= MSG_STATE_SENT_NO_ERR
        else:
            if (self.__didTheMessageRecipientExist(result) == False): state |= MSG_STATE_SENT_ERR_RECIPIENT_DOESNT_EXIST
            if (self.__didTheMessageHadASubject(result) == False):    state |= MSG_STATE_SENT_ERR_NO_SUBJECT
            if (self.__didTheMessageHadAText(result) == False):       state |= MSG_STATE_SENT_ERR_NO_TEXT
            if (self.__didTheMessageHadARecipient(result) == False):  state |= MSG_STATE_SENT_ERR_NO_RECIPIENT
            if (self.__blockedFromMessageRecipient(result) == True):  state |= MSG_STATE_SENT_ERR_BLOCKED
    ...
    def getSummaryOfMessageDeliveryStates(self):
        ...
        for msg in self.__sent:
            if   (msg.state & MSG_STATE_SENT_NO_ERR != 0): numberOfsuccessfulMessages += 1
            elif (msg.state & MSG_STATE_UNKNOWN != 0):     numberOfUnknownMessages += 1
            elif (msg.state & errorMask != 0):             numberOfFailedMessages += 1 
